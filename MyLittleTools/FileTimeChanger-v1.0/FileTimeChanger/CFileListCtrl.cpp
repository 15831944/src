...
void CFileListCtrl::InsertOneItem(FILE_LIST_ITEM fli) {
	...
	TCHAR tmp[1024] = { 0 };
	RtlZeroMemory(tmp, 1024 * sizeof(TCHAR));
...
void CFileListCtrl::InsertOneFile(LPTSTR szFilePath) {
	if (!PathFileExists(szFilePath)) {
		MesBox(this->m_hWnd, MB_ICONERROR | MB_OK, TEXT("ERROR"), TEXT("! No such file.\nFile Path: %s."), szFilePath);
		return;
	}

	FILE_TIMES* pft = new FILE_TIMES;
	if(pft == NULL){
		MesBox(this->m_hWnd, MB_ICONERROR | MB_OK, TEXT("ERROR"), TEXT("STH Wrong When malloc FILE_TIMES.\nErr Code : %d."), GetLastError());
		return;
	}
...
BOOL CFileListCtrl::IsRepeat(LPTSTR szPath) {
	BOOL bRet = FALSE;
	int iCount = this->GetItemCount();
	CString csTmp;

	for (int i = 0; i < iCount; i++) {
		csTmp = this->GetItemText(i, PATH);
		if (0 == lstrcmp(szPath, csTmp.GetString())) {
			bRet = TRUE;
			break;
		}
	}
	...
	if (hDropInfo)//拖拽消息指针
	{
		...
		for (int i = 0; i < nDrag; i++) {
			...
			if (FALSE == PathIsDirectory(szFilePath)) {
				if(FALSE == IsRepeat(szFilePath))
					InsertOneFile(szFilePath);
				else {
					MesBox(this->m_hWnd, MB_ICONERROR|MB_OK, TEXT("ERROR"), TEXT("A OU,<%s> is already in list."), szFilePath);
				}
			}
			else {
...
void CFileListCtrl::DelSelectedItem() {
